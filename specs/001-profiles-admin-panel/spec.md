...
- **FR-002**: The system MUST reject a skill whose name duplicates an existing skill, reporting the conflict against the name field.
...
- **FR-005**: Administrators MUST be able to edit an existing skill's name and category.
...
- **Skill**: A named capability in a curated platform-wide vocabulary, belonging to exactly one of four service categories (technology, design, writing, marketing). Names are unique. Created, corrected and removed only by administrators; freelancers and clients select from the list and never extend it. A skill has no active/inactive status — it exists or it does not — and it can be permanently removed only while no profile refers to it.
...
- **Two new business rules arrive with this feature**, and everything else surfaces rules that already govern the data: (1) a profile cannot be created for an account that is deactivated in the state being saved (FR-029), and (2) a skill that any profile still refers to cannot be removed (FR-028). Beyond these two, no new rule is introduced — this feature does not, for example, require a freelancer to declare a minimum number of skills.
...
- **The vocabulary starts populated**: The platform already ships a seeded baseline of 30 skills spread across the four service categories. User Story 1 is about growing and correcting that baseline, not about starting from an empty list.
