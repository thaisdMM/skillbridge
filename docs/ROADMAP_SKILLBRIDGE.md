...
- 🟡 **TASK 1.1.1** — PostgreSQL 17: patch level da imagem Docker não confirmado. A imagem `postgres:17` pode não estar na 17.10, que corrigiu 11 vulnerabilidades de segurança em 4 Jun 2026. Verificar a tag no Docker Hub quando o ambiente for atualizado (baseline §6).
- 🟡 **TASK 1.1.2** — `python:3.14-slim` base image: patch não fixado. Última série é 3.14.6 (10 Jun 2026); não bloqueante, mas vale confirmar junto com o upgrade do Django (baseline §1).
- 🟡 **TASK 1.1.3** — PostgreSQL 17 como service no CI workflow: mesmo risco de patch da TASK 1.1.1. Verificar se a tag usada alcança a 17.10 (baseline §6).
- 🟡 **SPRINT 1.2 (accounts/)** — `DEFAULT_AUTO_FIELD` não confirmado como declarado explicitamente em `settings.py`. Django 6.0 mudou o default de `AutoField` para `BigAutoField`; se o setting não estiver explícito, todos os modelos usam `BigAutoField` silenciosamente. Verificar se o comportamento é intencional (baseline §2, "DEFAULT_AUTO_FIELD now defaults to BigAutoField").
- 🟡 **SPRINT 2.1 (profiles/)** — `URLField`: Django 6.0 mudou o scheme padrão de `forms.URLField` de `'http'` para `'https'`. `models.URLField` é backed por `forms.URLField` em ModelForms e serializers DRF. Afeta `portfolio_url` (`FreelancerProfile`) e `website_url` (`ClientProfile`) — verificar o comportamento de validação de URLs sem scheme ao implementar os serializers (Sprint 3.2) (baseline §2, "Features removed in 6.0").
